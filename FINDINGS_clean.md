...
- **Downstream capability** (matched calib 64, real tasks): 1×1 keeps **~100% of ability through 20%, 98% at 30%, 82% at 50%** — at those *same doses* 32×32 retains only **43% / 23% / collapsed** (and is already down to 79% by just 10%). 1×1 crosses the 90%-retained bar (Finding 1's definition of the 5% point) at **~40%** (interpolated between the measured 30%→0.98 and 50%→0.82 points; a p40 1×1 downstream run is anchoring it directly) — so on the *same capability axis*, unstructured redundancy is **~8× larger** than the 32×32 ceiling.
...
**Numbers (Qwen3-4B, dense ppl 13.22 / retained 1.00):** 1×1+repair ppl 13.48 / 13.73 / 14.51 / 15.69 at 20/30/40/50% (32×32: 30.95 / 61.56 / 76.30 / 137.52). 1×1 retained-ability 1.00 / 1.00 / 0.98 / 0.82 at 10/20/30/50% (32×32: 0.79 / 0.43 / 0.23 / collapsed-at-50 *inferred* from ppl 137.5, no downstream run). Probe: harvestable-as-pure-tiles 5.0% → 0.02% → ~0 at T = 1 → 2 → ≥4 (practically diffuse).
